  ...
  return (
    <tr className='border-b-[1px] border-[#22262e] h-20 text-[1.1rem] font-semibold'>
      <td className='flex mt-6 gap-2 px-3 md:px-0'>
        ...
        <span className='text-[#9fa6b7] text-[15px] md:text-[20px] pr-4'>{symbol.toUpperCase()}</span>
      </td>
      <td className='px-2 md:px-0 text-[12px] md:text-[20px]'>{name}</td>
      <td className='px-3 md:px-0'>${current_price.toLocaleString()}</td>
      <td className={price_change > 0 ? 'text-green-500 px-3 md:px-0' : 'text-[#d33636] px-3 md:px-0'}>{price_change.toFixed(2)}%</td>
      <td className='px-3 md:px-0'>{total_volume.toLocaleString()}</td>
      <td className='px-3 md:px-0 w-16 flex justify-center items-center'> {/* Set a fixed width and use flexbox */}
        <img 
